...
packedblock_callback make_iter_obj(packedobj_callback cb) {
    return [cb](packedblock_ptr bl) {
        if (bl) {
...
class obj_iter {
    public:
        obj_iter(std::function<packedblock_ptr(void)> reader_) : reader(reader_), idx(0), ii(0) { curr = reader(); }
        
        std::pair<size_t,packedobj_ptr> next() {
    ...
    private:
        std::function<packedblock_ptr(void)> reader;
        size_t idx;
        size_t ii;
...
int find_difference(const std::string& left_fn, const std::string& right_fn, size_t numchan, std::function<bool(std::vector<objdiff>)> callback) {
    ...
    auto left_reader = std::make_shared<inverted_callback<packedblock>>([left_fn,numchan](packedblock_callback cb) { read_blocks<packedblock>(left_fn, cb, {}, numchan/2, nullptr, false, 7, false); });
    auto right_reader = std::make_shared<inverted_callback<packedblock>>([right_fn,numchan](packedblock_callback cb) { read_blocks<packedblock>(right_fn, cb, {}, numchan/2, nullptr, false, 7, false); });
    
    std::vector<objdiff> curr;
    curr.reserve(10000);
    auto left_obj = obj_iter([left_reader]() { return left_reader->next(); });
    auto right_obj = obj_iter([right_reader]() { return right_reader->next(); });
    ...
    while (left.second || right.second) {
        if (!left.second || (right.second->InternalId() < left.second->InternalId())) {
            ...
            if (left.second->InternalId()>next_obj) {
                std::cout << "\r[" << TmStr{ts.since(),7,1} <<"]" << left.second->Type() << " " << std::setw(10) << left.second->Id() << " [" << std::setw(10) << left.first << "//" << std::setw(10)  << right.first << ": " << std::setw(5)  << tot << "]" << std::flush;
            ...
            if (right.second->Data()!=left.second->Data()) {
