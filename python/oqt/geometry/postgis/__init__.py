...
def type_str(ct):
    if ct==_postgis.GeometryColumnType.BigInteger:
        return 'bigint'
    elif ct==_postgis.GeometryColumnType.Text:
        return 'text'
    elif ct==_postgis.GeometryColumnType.Double:
        return 'float'
    elif ct==_postgis.GeometryColumnType.Hstore:
        return 'hstore'
    elif ct==_postgis.GeometryColumnType.PointGeometry:
        return 'geometry(Point,3857)'
    elif ct==_postgis.GeometryColumnType.LineGeometry:
        return 'geometry(Linestring,3857)'
    elif ct==_postgis.GeometryColumnType.PolygonGeometry:
...
def write_to_postgis(prfx, box_in,connstr, tabprfx, stylefn=None, writeindices=True, lastdate=None,minzoom=None,nothread=False, numchan=4, minlen=0,minarea=5,extraindices=False,use_binary=True, extended=False):
    ...
    cnt,errs=None,None
    if nothread:
        errs = _postgis.process_geometry_postgis_nothread(params, postgisparams, Prog(locs=params.locs))
    else:
        errs = _postgis.process_geometry_postgis(params, postgisparams, None)#Prog(locs=params.locs))
...
def write_to_csvfile(prfx, box_in,outfnprfx,  stylefn=None, lastdate=None,minzoom=None,nothread=False, numchan=4, minlen=0,minarea=5, use_binary=True,extended=False):
    ...
    if nothread:
        ...
        cnt, errs = _postgis.process_geometry_csvcallback_nothread(params, postgisparams, Prog(locs=params.locs), csvwriter)
        
    else:
        if outfnprfx is None:
            
            csvwriter=CsvWriter(outfnprfx,len(params.locs)>100)
            cnt, errs = _postgis.process_geometry_csvcallback(params, postgisparams, Prog(locs=params.locs),csvwriter)
        else:
            cnt, errs = _postgis.process_geometry_csvcallback_write(params, postgisparams, Prog(locs=params.locs),outfnprfx)
    #if writeindices:
