...
def find_change(src_filenames, prfx, infiles, startdate, enddate, outfn, use_alt=False,allow_missing_users=False):
    ...
    tm("find orig")
    _oqt.calc_change_qts(objs,qts)
    tm("calc qts")

    tiles = _oqt.find_change_tiles(objs,orig_allocs,tree,startdate,enddate)
    ...
    out = _oqt.WritePbfFile(prfx+outfn,bounds=_oqt.bbox(-1800000000,-900000000,1800000000,900000000), numchan=4, indexed=True, dropqts=False, change=True, tempfile=False)
